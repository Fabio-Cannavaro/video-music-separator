...
| GitHub 인증 | 현재 비공개 테스트 빌드는 GitHub CLI가 필요하다. 로그인이 없거나 만료되었으면 설치 프로그램이 GitHub 웹 인증을 시작한다. 공개 배포본에서는 이 요구사항을 제거할 예정이다. |
...
6. 로그인이 없거나 만료되었다면 설치 프로그램이 여는 GitHub CLI 창과 웹브라우저에서 이 저장소에 접근할 수 있는 계정으로 인증한다. 일회용 코드는 클립보드에 자동 복사된다.
...
1. 다음 항목을 지정 배포처에서 내려받는다.
   - AI Python 실행환경: 현재 비공개 테스트에서는 로컬 GitHub CLI 인증으로 이 프로젝트의 비공개 Release에 고정된 두 분할 파일
...
- `GitHub CLI를 찾을 수 없습니다` 오류가 나오면 GitHub CLI를 설치한 뒤 설치 프로그램을 다시 실행한다. 로그인이 없거나 만료된 경우에는 설치 프로그램이 웹 인증을 자동으로 시작한다. 현재 비공개 테스트에서는 `Fabio-Cannavaro/video-music-separator`를 볼 수 있는 계정이어야 한다.
