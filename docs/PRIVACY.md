...
| FFmpeg GPL Essentials 빌드 | `www.gyan.dev` | 고정된 Gyan FFmpeg 9.0.1 배포 파일 다운로드 |
...
설치 파일은 AI 실행환경·모델과 FFmpeg 9.0.1의 URL, 예상 파일 크기와 SHA-256을 first-party 잠금값으로 확인한다. FFmpeg 세 실행 파일도 실행 전에 개별 크기와 SHA-256을 확인하고 GPL Essentials 정적 빌드 옵션을 검증한다. 설치 완료 후 일반적인 영상 처리에는 인터넷 연결이 필요하지 않다.

일반 영상 처리 입력은 로컬 고정 디스크의 지원 컨테이너 형식으로 제한한다. UNC·네트워크 드라이브·재분석 지점·재생목록과 FFmpeg 네트워크 프로토콜은 입력으로 허용하지 않는다.

## 로컬 파일

- 원본 영상과 음원은 사용자가 선택한 위치에서 읽는다.
- 임시 분리 파일은 원본 영상 옆의 실행별 `<영상이름>_sound_work_<난수>` 폴더에 저장한다.
- 정상적으로 사본 저장과 검증이 끝나면 임시 작업 폴더를 삭제한다.
- 저장 실패나 검증 실패가 발생하면 복구와 진단을 위해 임시 파일이 남을 수 있다.
- 설치된 구성요소의 출처·버전·체크섬은 앱 폴더의 `docs/runtime-assets.json`에 기록한다.
