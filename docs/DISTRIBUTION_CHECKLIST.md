...
- [x] 최신 설치 테스트본과 필수 AI 런타임만 프리릴리스로 공개하고, 구형 설치본은 Draft로 보존한다.
...
- [ ] 앱과 설치 파일에서 영상·음원이 외부로 전송되지 않는지 다시 검사한다.
  ...
  - [ ] 모든 FFmpeg 계열 입력을 로컬 파일 프로토콜로 제한하고, 조작된 재생목록의 외부 요청 가능성을 제거한 뒤 다시 검사한다.
...
- [ ] 기존 AI 런타임과 FFmpeg를 재사용하기 전에 앱에 고정된 파일별 SHA-256 목록으로 검증하고, 신뢰하지 않은 실행 파일을 검증 목적으로 먼저 실행하지 않는다.
- [ ] `<영상이름>_sound_work`가 이번 앱 작업에서 만든 폴더임을 소유권 표식으로 확인하고, 기존의 무관한 폴더나 파일을 재사용·삭제하지 않는다.
