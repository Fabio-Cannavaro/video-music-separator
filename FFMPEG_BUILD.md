...
앱 빌드 시 `prepare_ffmpeg_lgpl.ps1`가 위 고정 파일을 내려받고 SHA-256을 확인한다. `build_portable.ps1`는 `--enable-shared`가 있고 `--enable-gpl`, `--enable-nonfree`가 없는지 다시 확인한 뒤 EXE와 필요한 DLL을 함께 복사한다.
